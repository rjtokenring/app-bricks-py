...
#
# Same idea as qairt-common-base's qairt-entrypoint.sh, but for the Debian
# libfastrpc1 this image installs instead of the fastrpc source build: that
# build reads its config from /usr/share/hexagon-dsp/conf.d and resolves each
# DSP_LIBRARY_PATH relative to /usr/share/hexagon-dsp (the QAIRT source build
# uses /usr/share/qcom for both).
#
# Installed at /provision-fastrpc-dsp.sh and run by /aihub-onnx-entrypoint.sh
