...
#
# Runs from the repository against the source tree, or inside the built image -
# `docker run -v "$PWD/tests:/t:ro" --entrypoint bash <image> /t/...` - where it
# exercises the copies the image actually ships.
set -eu

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
# The installed copy when there is one (i.e. inside the image), else the source tree.
SCRIPT=/provision-fastrpc-dsp.sh
[ -f "$SCRIPT" ] || SCRIPT="$SCRIPT_DIR/../scripts/provision-fastrpc-dsp.sh"
[ -f "$SCRIPT" ] || { echo "FAIL: provision-fastrpc-dsp.sh not found ($SCRIPT)"; exit 1; }
...
# Baked file the script copies from: the installed one when running inside the
# image, else the source tree's.
