...
#
# Verbatim copy of containers/base/qairt-common-base/qairt-entrypoint.sh: this
# image derives from the Edge Impulse QNN runtime, not from qairt-common-base,
# and each container is built with its own directory as the build context, so
# the shared script cannot be COPYied from here. Both builds of fastrpc read
# their config from /usr/share/qcom, hence the same script.
# tests/test-qairt-entrypoint.sh fails if the two drift apart.
set -eu
