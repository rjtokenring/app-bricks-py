...
"""API/signature stability tests for the LargeLanguageModel inheritance chain.

These tests guard the public contract of ``LargeLanguageModel`` and the
constructor "hand-off" to its parent class ``CloudLLM``.
...
def test_cloud_llm_init_accepts_var_keyword_passthrough():
    """LargeLanguageModel forwards ``**kwargs`` upward, so CloudLLM must accept ``**kwargs``."""
    sig = inspect.signature(CloudLLM.__init__)
    assert any(p.kind is inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()), "CloudLLM.__init__ must accept **kwargs"


# ---------------------------------------------------------------------------
# Public API surface of LargeLanguageModel.
# ---------------------------------------------------------------------------
...
def test_llm_public_methods_are_present():
    for name in EXPECTED_LLM_PUBLIC_METHODS:
        assert callable(getattr(LargeLanguageModel, name, None)), f"LargeLanguageModel.{name} is missing"


def test_llm_provider_prefix_constants_are_stable():
    """Local provider prefixes drive base_url/host routing in __init__."""
    assert LargeLanguageModel.GENIE_MODEL == "genie"
    assert LargeLanguageModel.LLAMACPP_MODEL == "llamacpp"


# Expected signature fingerprints for the LLM public API. Update deliberately
# (with a matching changelog entry) whenever the public contract changes.
_P = inspect.Parameter
EXPECTED_LLM_SIGNATURES = {
    "__init__": [
...
@pytest.mark.parametrize("method_name", sorted(EXPECTED_LLM_SIGNATURES))
def test_llm_public_signatures_are_stable(method_name):
    assert _signature_fingerprint(LargeLanguageModel, method_name) == EXPECTED_LLM_SIGNATURES[method_name]


def test_llm_inheritance_chain_is_intact():
    """LargeLanguageModel must remain a CloudLLM subclass for the tests above to be meaningful."""
