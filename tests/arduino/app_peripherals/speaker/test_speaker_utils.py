...
class TestPipeWireUnavailable:
    """With pw-dump unusable, speaker discovery degrades to ALSA-only enumeration."""

    def test_low_level_discovery_still_reports_the_failure(self, mock_pw_dump):
        # list_audio_sinks keeps its contract: the fallback lives above it.
    ...
    def test_usb_cards_are_selected_by_index(self, mock_pw_dump, usb_cards):
        ...
        assert _nth_plugged_speaker(0) == "usb:1"
        assert _nth_plugged_speaker(1) == "usb:2"
    ...
    def test_non_usb_cards_are_offered_as_last_resort(self, mock_pw_dump, usb_cards):
        # No USB card at all: a built-in codec is better than no speaker.
        mock_pw_dump(unavailable=True)
        usb_cards()
    ...
    def test_last_resort_skips_cards_that_are_not_speakers(self, mock_pw_dump, usb_cards):
        # Playing into an HDMI or loopback card would be silent, not audible.
        mock_pw_dump(unavailable=True)
        usb_cards()
    ...
    def test_claim_skips_already_claimed_fallback_speakers(self, mock_pw_dump, usb_cards):
        mock_pw_dump(unavailable=True)
        usb_cards(0, 1)
    ...
    def test_jack_is_not_offered_under_media_carrier(self, mock_pw_dump, usb_cards, monkeypatch):
        # Jack speakers are PipeWire nodes: without PipeWire they can't be opened.
        monkeypatch.setenv(_CARRIER_ENV, "media-carrier")
        mock_pw_dump(unavailable=True)
        usb_cards(0, 1)

        with pytest.raises(SpeakerOpenError):
            _nth_plugged_speaker(2)
